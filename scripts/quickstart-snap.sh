...
if [ -n "${SNAP_USER_DATA}" ]; then
  prefix="${SNAP_USER_DATA}/"
fi

app_name="quickstart"
app_git_repo_owner="akarnil"
app_git_branch="snap-support"


cn="device"
private_key_path="${prefix}${cn}-pkey.pem"
cert_key_path="${prefix}${cn}-cert.pem"
config_json_path="${prefix}iotcDeviceConfig.json"
quickstart_app_path="${prefix}${app_name}.py"
...
function gencert {
  ...
  openssl ecparam -name ${ec_curve} -genkey -noout -out ${private_key_path}
  openssl req -new -days ${days} -nodes -x509 \
      -subj "${subj}" -key ${private_key_path} -out ${cert_key_path}

  echo "X659 credentials are now generated as ${cert_key_path} and ${private_key_path}."
}
...
if [[ -f "${cert_key_path}" && -f "${private_key_path}" ]]; then
...
if [[ ${do_download} && -f ${quickstart_app_path} ]]; then
...
if ${do_download}; then
  ...
  if [[ -z "${QUICKSTART_PY_URL}" ]]; then
    QUICKSTART_PY_URL="https://raw.githubusercontent.com/${app_git_repo_owner}/iotc-python-lite-sdk/refs/heads/${app_git_branch}/examples/${app_name}.py"
  fi

  echo "Downloading the quickstart Python source from ${QUICKSTART_PY_URL}..."

  if ${use_curl}; then
    curl --silent --output ${quickstart_app_path} "${QUICKSTART_PY_URL}"
  else
    wget -O "${quickstart_app_path}" -qN "${QUICKSTART_PY_URL}"
  fi
